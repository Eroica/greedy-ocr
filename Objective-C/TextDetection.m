...
// NSMutableArray *
// findBoundingBoxes(NSMutableArray *components,
//                   NSMutableArray *chains,
//                   NSMutableArray *compBB,
//                   IplImage *output)
// {
//     NSMutableArray *bb = [NSMutableArray arrayWithCapacity:[chains count]]
...
//             maxy = fmax(maxy, compBB[*cit].second.y);
//             maxx = fmax(maxx, compBB[*cit].second.x);
//         }
//     }
              ...
              bool dark_on_light)
{
    ...
    NSLog(@"Running textDetection with dark_on_light = %d\n", dark_on_light);

    IplImage *grayImage = cvCreateImage(cvGetSize(input),
                                        IPL_DEPTH_8U,
                                        1);
    ...
    IplImage *edgeImage = cvCreateImage(cvGetSize(input),
                                        IPL_DEPTH_8U,
                                        1);

    cvCanny(grayImage, edgeImage, threshold_low, threshold_high, 3) ;
    cvSaveImage("canny.png", edgeImage, 0);

    /* Create gradient X, gradient Y. */
    IplImage *gaussianImage = cvCreateImage(cvGetSize(input),
                                            IPL_DEPTH_32F,
                                            1);
    cvConvertScale(grayImage, gaussianImage, 1./255., 0);
    // cvSmooth(gaussianImage, gaussianImage, CV_GAUSSIAN, 5, 5);
    cvSmooth(gaussianImage, gaussianImage, CV_GAUSSIAN, 5, 5, 0, 0);

    IplImage *gradientX = cvCreateImage(cvGetSize(input),
                                        IPL_DEPTH_32F,
                                        1);
    IplImage *gradientY = cvCreateImage(cvGetSize(input),
                                        IPL_DEPTH_32F,
                                        1);
