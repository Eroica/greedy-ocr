...
cv::Mat
prepareImage(cv::Mat input_image, bool dark_on_light) {
    ...
    cv::Mat gray_image(input_image.rows, input_image.cols, input_image.type());
    input_image.copyTo(gray_image);
    cvtColor(gray_image, gray_image, cv::COLOR_RGB2GRAY);

    // Canny.
    double threshold_low = 175;
    double threshold_high = 320;
    cv::Mat edge_image(input_image.rows, input_image.cols, input_image.type());
    cv::Canny(gray_image, edge_image, threshold_low, threshold_high);
    imwrite("canny.png", edge_image);

    // Gradient.
    cv::Mat gaussian_image(input_image.rows, input_image.cols, input_image.type());
    cv::GaussianBlur(input_image, gaussian_image, 0, 5);

    cv::Mat gradientX(input_image.rows, input_image.cols, input_image.type());
    cv::Mat gradientY(input_image.rows, input_image.cols, input_image.type());

    cv::Sobel(gaussian_image, gradientX, gradientX.depth(), 1, 0);
    cv::Sobel(gaussian_image, gradientY, gradientY.depth(), 0, 1);
